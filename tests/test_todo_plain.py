...
with TestClient(app) as client:
    ...
    def test_read_all_todos_no_auth(set_env_vars_plain):
    ...
    def test_create_todo(set_env_vars_plain):
        access_token = login()
        response = client.post(
            "/api/todo/",
    ...
    def test_update_todo(set_env_vars_plain):
        access_token = login()
        response = client.put(
            f"/api/todo/{id}",
    ...
    def test_read_todo(set_env_vars_plain):
        access_token = login()
        response = client.get(
    ...
    def test_read_all_todos(set_env_vars_plain):
        access_token = login()
        response = client.get(
    ...
    def test_read_all_todos_with_cookie(set_env_vars_encrypted):
        ...
        response = client.get("/api/todo/", cookies=cookies)
        assert response.status_code == 200
        assert response.json() == {
            id: {
                "id": id,
    ...
    def test_update_todo_not_found(set_env_vars_plain):
        access_token = login()
        response = client.put(
            "/api/todo/test",
            json={"title": "test2", "done": True, "colour": "blue"},
    ...
    def test_delete_todo_not_found(set_env_vars_plain):
        access_token = login()
        response = client.delete(
    ...
    def test_read_todo_not_found(set_env_vars_plain):
        access_token = login()
        response = client.get(
            "/api/todo/test", headers={"Authorization": f"Bearer {access_token}"}
        )
